...
public static class Helper
{
    ...
    public static InforPointByHit GetObjectByTouch(Vector2 position)
    {
        var cam = Camera.main;
        if (cam == null) return null;
        Ray ray = Camera.main.ScreenPointToRay(position);
        RaycastHit[] raycastHits;
        raycastHits = Physics.RaycastAll(ray);
        InforPointByHit inforPointByHit;
        int i = 0;
        foreach (RaycastHit hit in raycastHits)
        {
            inforPointByHit = new InforPointByHit();
            if (hit.collider.transform.root.gameObject.tag == TagConfig.ORGAN_TAG)
            {
                // inforPointByHit.hitPoint = hit.collider.gameObject.transform.InverseTransformPoint(hit.point);
                inforPointByHit.hitPoint = hit.point;
                Debug.Log($"quyennt57 : {inforPointByHit.hitPoint}");
                if (hit.collider.gameObject.transform.parent == ObjectManager.Instance.CurrentObject.transform)
                {
                    inforPointByHit.ObjectByHit = hit.collider.gameObject;
                }
                else
                {
                    inforPointByHit.ObjectByHit = null;
                }
                return inforPointByHit;
            }
        }
        return null;
    }
    ...
    public static Bounds CalculateBoundsOneLevel(GameObject obj)
    {
        List<Renderer> renderers = new List<Renderer>();
        foreach(Transform item in obj.transform)
        {
            if (item.GetComponent<Renderer>() && item.gameObject.tag == TagConfig.ORGAN_TAG)
